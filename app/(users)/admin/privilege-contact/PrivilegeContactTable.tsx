...
function PrivilegeContactModal({
  row,
  onClose,
}: {
  row: PrivilegeContactRow;
  onClose: () => void;
}) {
...
function PrivilegeContactRowItem({
  row,
  onDelete,
}: {
  row: PrivilegeContactRow;
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/admin/privilege-contact/${row.id}`, {
        method: "DELETE",
      });
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3.5">
        ...
        {open ? <PrivilegeContactModal row={row} onClose={close} /> : null}
      </td>
    </tr>
  );
}

export function PrivilegeContactTable({
  rows: initialRows,
}: {
