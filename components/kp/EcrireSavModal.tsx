...
export function EcrireSavModal({ triggerClassName, children }: EcrireSavModalProps) {
	...
	async function handleSubmit(e: FormEvent<HTMLFormElement>) {
		...
		if (!contactTrim) {
			setError("Indiquez un moyen de contact (téléphone ou e-mail).");
			return;
		}
		if (!modeleTrim) {
			setError("Indiquez le modèle de votre véhicule.");
			return;
		}
		if (!panneTrim) {
			setError("Décrivez la panne ou le besoin.");
			return;
		}
		if (!localisationTrim) {
			setError("Indiquez votre localisation.");
			return;
		}
						...
						<div className="flex items-start justify-between gap-4 border-b border-white/10 px-6 py-5 sm:px-10 sm:py-6">
							<div>
								...
								<p className="mt-2 font-sans text-sm text-white/50">
									Décrivez votre besoin — notre équipe vous recontacte sous 24 h.
								</p>
							</div>
							<button
								type="button"
						...
						{isSuccess ? (
							...
							>
								...
								<p className="mx-auto mt-4 max-w-sm text-[14px] leading-relaxed text-white/50">
									Merci. Nous avons bien reçu votre demande et vous recontacterons
						...
						>
							...
							<div>
								<label htmlFor="ecrire-sav-contact" className={labelClass}>
									Contact <span className="text-kp-gold/90">*</span>
								</label>
								<input
									...
									placeholder="+225 07 … ou vous@exemple.com"
									className={inputClass}
								/>
							</div>
							...
							<div>
								<label htmlFor="ecrire-sav-modele" className={labelClass}>
									Modèle de véhicule <span className="text-kp-gold/90">*</span>
								</label>
								<select
							...
							<div>
								<label htmlFor="ecrire-sav-panne" className={labelClass}>
									Panne / besoin <span className="text-kp-gold/90">*</span>
								</label>
								<textarea
									id="ecrire-sav-panne"
									name="panne"
									...
									placeholder="Décrivez la panne ou le type d’intervention souhaitée…"
							...
							<div>
								<label htmlFor="ecrire-sav-localisation" className={labelClass}>
									Localisation <span className="text-kp-gold/90">*</span>
								</label>
								<input
