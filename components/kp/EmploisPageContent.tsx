...
export function EmploisPageContent() {
  const { tr } = useLocale();

  const APPLICATION_CHIPS = [
    tr("Formulaire CV & compétences", "CV & skills form"),
    tr("PJ PDF recommandée", "PDF attachment recommended"),
    tr("Contact RH", "HR contact"),
  ] as const;
      ...
      <main className="relative overflow-hidden pt-[110px] md:pt-[132px]">
        ...
        <div className="relative z-10 mx-auto max-w-[1600px] px-4 pb-24 sm:px-5 md:px-10 md:pb-28 lg:pb-36">
          <Reveal rootMargin="-40px">
            <EmploisPageHero
              kicker={tr("Rejoignez l'équipe", "Join the team")}
              title={tr("Candidature", "Application")}
              description={tr(
                "Partagez votre parcours, vos compétences et une synthèse de votre CV. Une fois le brouillon e-mail créé, ajoutez votre pièce jointe PDF avant validation.",
                "Share your background, your skills and a summary of your résumé. Once the email draft is created, attach your PDF before sending."
              )}
                ...
                <aside className="relative lg:sticky lg:top-[calc(132px+1.25rem)] lg:self-start lg:pl-8 xl:top-[calc(132px+1.75rem)]">
                        ...
                        <div className="relative flex items-start gap-4">
                          ...
                          <div className="min-w-0 text-left">
                            <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.28em] text-white/38">
                              {tr("CV lisible & court", "A clear, concise résumé")}
                            </p>
                            <p className="mt-2 text-[13px] leading-relaxed text-white/42">
                              {tr(
                                "En deux ou trois phrases dans le champ dédié, mettez en avant ce qui nous aidera à comprendre votre profil avant d'ouvrir votre PDF.",
                                "In two or three sentences in the dedicated field, highlight what will help us understand your profile before opening your PDF."
                              )}
                            </p>
                          </div>
                        </div>
                    ...
                    <li className="min-w-0">
                      <div className={cardClass} role="presentation">
                        ...
                        <div className="relative flex items-start gap-4">
                          ...
                          <div className="min-w-0 text-left">
                            <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.28em] text-white/38">
                              {tr("Compétences ciblées", "Targeted skills")}
                            </p>
                            <p className="mt-2 text-[13px] leading-relaxed text-white/42">
                              {tr(
                                "Reliez explicitement vos savoir-faire au poste visé ; les langues et les outils métiers comptent.",
                                "Explicitly connect your skills to the target role; languages and job-specific tools matter."
                              )}
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="mt-6 rounded-2xl border border-white/6 bg-black/25 p-5 backdrop-blur-sm sm:mt-8 sm:p-6">
                    <p className="text-center font-sans text-[10px] font-semibold uppercase tracking-[0.28em] text-white/35 lg:text-left">
                      {tr("Besoin d'échanger avant d'envoyer ?", "Need to talk before sending?")}
                    </p>
                    <p className="mt-3 text-center text-[13px] leading-relaxed text-white/42 lg:text-left">
                      {tr(
                        "Pour une question générale ou un premier contact, vous pouvez aussi nous écrire depuis la page Contact.",
                        "For a general question or an initial contact, you can also write to us from the Contact page."
                      )}
                    </p>
                    <Link
                      href="/contact"
                      ...
                      {tr("Aller à la page Contact", "Go to the Contact page")}
                    </Link>
                  </div>
                </aside>
              </Reveal>
